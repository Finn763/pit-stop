...
- 问题：没有"一句话改进整个项目"的全闭环 skill。PR 评审 agent 只审 PR；
  单文件评审 skill 只诊断不执行；superpowers 只有阶段、无端到端配方；
  mp 的 improve-codebase-architecture 止于"出报告+烤问"，不动手。
- 非目标：不做独立 CLI；不拆多 skill（跑顺 3 个项目再拆，学 ponytail 的 6 件套演化路径）；
...
1. **装载**：读 README/AGENTS.md（或同类）+ `git status` + 最近 20 commits 热区。
   产物：项目一句话 + 脏区清单。（学 mp：Scope before you scan，近改动权重优先）
   开工前先过 MODE 表（学 BugHunter）：写下"什么算 finding、什么直接拒"——缺关键信息问一次，
...
4. **执行**：遵守 §4 护栏。单轮修完派独立 reviewer 复查（学 ToB：reviewer 与 fixer 分离，
   复查最好换模型/换视角）；修→查循环直到干净或撞 `--max-rounds`（默认 3 轮）上限；
   跨轮 findings ledger 防重复修同一条；连续两轮不收敛 → 升级为"需人定"停手
   （学 ToB escalation）。失败（测试红/撞禁区/单阶段超 $20）→ 停手，写进 §5 未做区，不硬猜不绕路。
5. **汇报**：头标注"本报告由 pit-stop 全自动生成"；固定四块——改了什么 / 验过什么（工具输出）
   / 没验什么 / 剩下什么（需人定）。禁止无工具输出的口头成功。

## 4. 判断与约束机制（三层，学的 git-guardrails：能强制的不只写文档）
...
- **claims 审计（学 tides-review 签名动作）**：注释、文档、测试名里的每个保证都是 CLAIM，
  必须找到代码里兑现它的路径；兑现不了的就是 finding。
- **证据卫生（学 BugHunter evidence-hygiene）**：报告、日志、终端摘录里只放"说服人"的信息；
  密钥、token、他人 PII、内网路径一律打码或删掉，和 L3 终扫是同一条线的两道锁（一道习惯、一道机器）。
- **验证门（学 superpowers verification）**：IDENTIFY→RUN→READ→VERIFY 四步，跳步 = 说谎；
  汇报分"验过/没验"两区；信任子代理自报前先看 diff（子代理自报成功≠成功）。
- **双轴复核（学 mp code-review）**：执行后 diff 过两轴——规范轴（跟仓库既有风格）+ 需求轴（跟本轮建议），
...
- frontmatter 照抄 ToB：`name` + `description`（触发条件+关键词轰炸，学 sanyuan skill-review：
  多种 phrasing 同一意图）+ `argument-hint: "<项目路径> [--禁区 ...]"` + `allowed-tools` 白名单。
- 汇报铁律（学 sanyuan IRON LAW）：不许"可以优化"而不说改哪、为什么——每条建议必须可执行。
- 关键词覆盖：improve/refactor/audit/tech-debt/review/verify/guardrail（SDO：按症状词写）。

## 6. 验证计划（writing-skills TDD + ponytail 式诚实数字）

- 基线：2 个真实项目跑"无 skill 会怎么干"，记录瞎搞点（red）。
- 定稿重跑：证据率 / 测试门遵守率 / 口头成功零容忍（green），补 rationalization 表。
- 发布数字必须诚实：注明 baseline、样本数、模型，像 ponytail 区分 single-shot 和 agentic 那样，
  不许拿单次最优当平均。
- 每次实战回来只补实测到的条目，不预写。
...
- 发布清单（学 cr-graph/ponytail）：多语言 README（至少中英）、benchmark 可复现说明、
